...
** Youtube
=ytel= stuff; very experimental.
#+BEGIN_SRC emacs-lisp
  (load-file "~/documents/src/ytel/ytel.el")
  (require 'ytel)

  (add-hook 'ytel-mode-hook (lambda ()
			      (hl-line-mode 1)
  ...
  (defun ytel-explore-element ()
    "Just print the current event as a message."
    (interactive)
    (let* ((video (ytel-get-current-video)))
      (message "%s" video)))
  (define-key ytel-mode-map "x" #'ytel-explore-element)
#+END_SRC
